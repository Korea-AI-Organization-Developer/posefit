...
// getExerciseFeedbacks: 지정한 운동 종목의 "당일(오늘, KST)" 세션 피드백만 가져온다.
//   GET /api/v1/exercises/{id}/feedbacks → Feedback[] (정렬: id ASC).
//   오늘 기록이 없으면 빈 배열([])을 돌려준다. (운동 종목이 없으면 백엔드가 404 → ApiError)
export async function getExerciseFeedbacks(id: number): Promise<Feedback[]> {
  return apiFetch(`/exercises/${id}/feedbacks`);
}