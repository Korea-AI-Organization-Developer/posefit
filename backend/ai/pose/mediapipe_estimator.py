"""
MediaPipe 포즈 추정 클래스 (run_mediapipe_v2_gpu.py 기반).
...
class MediaPipePoseEstimator:
    ...
    def run(self) -> dict:
        ...
        raw_json_path  = video_out_dir / "raw.json"
            ...
            while cap.isOpened():
                ...
                for person_lms in res.pose_landmarks:
                    ...
                    raw_person = {
                        "keypoints": {
                            COCO_KP_NAMES[i]: {
                                "x":          round(coco_pts[i][0], 6),
                                "y":          round(coco_pts[i][1], 6),
                                "z":          round(coco_pts[i][2], 6),
                                "visibility": round(coco_pts[i][3], 4),
                                "presence":   round(coco_pts[i][4], 4),
                            }
                            for i in range(17)
                        },
                        "angles": {
                            k: round(v, 4) if v is not None else None
                            for k, v in _compute_angles(coco_pts).items()
                        },
                    }
                ...
                raw_frame["pose_detected"]  = len(res.pose_landmarks) > 0
