...
def _window_duration_seconds(window: Dict[str, Any]) -> Optional[float]:
    time_range = window.get("time_range") or {}
    start_sec = time_range.get("start_sec")
    end_sec = time_range.get("end_sec")
    if _is_number(start_sec) and _is_number(end_sec):
...
def _score_window_errors(errors: List[Dict[str, Any]]) -> Dict[str, Any]:
    valid_errors = [error for error in errors if isinstance(error, dict)]
    penalties = []

    for error in valid_errors:
        severity = str(error.get("severity") or "unknown").lower()
        penalty = SEVERITY_SCORE_PENALTIES.get(
            severity,
            SEVERITY_SCORE_PENALTIES["unknown"],
        )
        penalties.append({
            "error_code": error.get("error_code") or error.get("issue"),
            "severity": severity,
            "penalty": _round(penalty, 3),
        })

    total_penalty = min(100.0, sum(float(item["penalty"]) for item in penalties))
    accuracy_pct = 100.0 if not valid_errors else 0.0
    score_pct = max(0.0, 100.0 - total_penalty)

    return {
        "accuracy_pct": _round(accuracy_pct, 3),
        "score_pct": _round(score_pct, 3),
        "score_breakdown": {
            "base_score": 100.0,
            "total_penalty": _round(total_penalty, 3),
            "penalties": penalties,
        },
    }
...
def _build_score_summary(window_results: List[Dict[str, Any]]) -> Dict[str, Any]:
    scored_windows = [
        window
        for window in window_results
        if isinstance(window, dict) and _is_number(window.get("accuracy_pct"))
    ]
    window_count = len(scored_windows)
    accuracy_values = [float(window["accuracy_pct"]) for window in scored_windows]
    score_values = [
        float(window.get("score_pct"))
        for window in scored_windows
        if _is_number(window.get("score_pct"))
    ]
    weighted_pairs = [
        (
            float(window["accuracy_pct"]),
            float(window.get("score_pct")),
            duration,
        )
        for window in scored_windows
        for duration in [_window_duration_seconds(window)]
        if duration is not None and _is_number(window.get("score_pct"))
    ]

    average_accuracy = _average(accuracy_values)
    average_score = _average(score_values)
    time_weighted_accuracy = _weighted_average(
        [pair[0] for pair in weighted_pairs],
        [float(pair[2]) for pair in weighted_pairs],
    )
    time_weighted_score = _weighted_average(
        [pair[1] for pair in weighted_pairs],
        [float(pair[2]) for pair in weighted_pairs],
    )
    correct_count = sum(1 for window in scored_windows if window.get("status") == "correct")

    return {
        "window_count": window_count,
        "correct_window_count": correct_count,
        "needs_correction_window_count": window_count - correct_count,
        "overall_accuracy_pct": _round(average_accuracy, 3),
        "average_window_accuracy_pct": _round(average_accuracy, 3),
        "average_window_score_pct": _round(average_score, 3),
        "score_pct": _round(average_score, 3),
        "time_weighted_accuracy_pct": _round(time_weighted_accuracy, 3),
        "time_weighted_score_pct": _round(time_weighted_score, 3),
        "scoring_method": {
            "accuracy_pct": "100 for a correct window, 0 for a window with any detected rule error.",
            "score_pct": "100 minus severity penalties per detected window error.",
            "severity_penalties": SEVERITY_SCORE_PENALTIES,
        },
    }
...
def _build_window_results(
    ...
    for index, segment in enumerate(segment_features):
        ...
        score_fields = _score_window_errors(errors)
        window_results.append({
            "window_id": window_id,
            "segment_id": segment.get("segment_id"),
            "status": "needs_correction" if errors else "correct",
            "accuracy_pct": score_fields["accuracy_pct"],
            "score_pct": score_fields["score_pct"],
            "score_breakdown": score_fields["score_breakdown"],
...
def pose_decide_node(state:FeedbackState) -> dict:
    ...
    score_summary = _build_score_summary(window_results)
    window_errors = [
        error
        for window in window_results
...
def _build_window_evaluations(
    ...
    for window in windows:
        ...
        evaluations.append({
            ...
            "status": status,
            "accuracy_pct": window.get("accuracy_pct"),
            "score_pct": window.get("score_pct"),
    ...
    for entry in timeline_entries:
        ...
        evaluated_errors = _window_errors_for_evaluation(errors, docs_by_error_code)
        score_fields = _score_window_errors(evaluated_errors)
        evaluations.append({
            "window_id": None,
            "segment_id": None,
            "status": "needs_correction" if evaluated_errors else "correct",
...
def _build_plank_refine_prompt(
    ...
    system_prompt = (
        ...
        "## 역할과 제약\n"
        "- 입력된 window별 평가 결과와 RAG 코칭팁만 사용해 최종 피드백을 작성합니다.\n"
        "- 자세 판단은 절대 새로 하지 않습니다. 제공된 오류와 시간 구간만 사용합니다.\n"
        "- 없는 시간 구간이나 새로운 오류를 생성하지 않습니다.\n\n"
        "## 출력 형식\n"
        "반드시 아래 JSON 스키마를 따르세요.\n\n"
        '{"summary": "세트 전체를 1~2문장으로 요약", '
        '"coaching": "시간 순서대로 작성한 구간별 피드백 (전체 서술형)", '
        '"next_action": "다음 세트에서 가장 중요하게 개선할 점 1가지", '
        '"timeline_feedback": [{"start_sec": 0.0, "end_sec": 3.0, '
    ...
    user_prompt = (
        ...
        f"## 작성 규칙\n"
        f"1. coaching은 반드시 시간 순서대로 작성하세요.\n"
...
def _refine_set_feedback_text(
    ...
    try:
        ...
        print(f"[DEBUG] _refine_set_feedback_text LLM 실패: {e}")
        return fallback
...
def _generate_set_feedback(
    ...
    score_summary = analysis_result.get("score_summary") or _build_score_summary(window_evaluations)
...
def daily_feedback_node(state: FeedbackState) -> dict:
    """today_set_results를 집계해 daily_feedback 구조체를 만든다 (LLM 없음)."""
    print("\n" + "="*60)
    print("[daily_feedback_node] 진입")
    today_set_results = state.get("today_set_results") or []
    print(f"  ▶ today_set_results 수신: {len(today_set_results)}건")
    for i, item in enumerate(today_set_results):
        print(f"    세트{i+1}: set_number={item.get('set_number')}, score={item.get('score')}, content={str(item.get('content',''))[:60]}")

    if not today_set_results:
        print("  ✗ today_set_results가 비어 있음 → 빈 daily_feedback 반환")
        print("="*60)
        return {"daily_feedback": {"total_sets": 0, "avg_score": None, "score_trend": [], "score_direction": "stable", "set_summaries": []}}
    ...
    if len(scores) >= 2:
        ...
        score_direction = "improving" if diff >= 5 else ("declining" if diff <= -5 else "stable")
    else:
    ...
    print(f"  ✓ 집계 완료: 총 {len(today_set_results)}세트 | 평균점수={avg_score} | 방향={score_direction} | 점수추이={scores}")
    print("="*60)
    return {
        "daily_feedback": {
...
def daily_text_summarize_node(state: FeedbackState) -> dict:
    """CoT 방식으로 평가+코칭을 통합한 일일 자연어 피드백을 생성한다 (LLM 1회)."""
    print("\n" + "="*60)
    print("[daily_text_summarize_node] 진입")
    daily_feedback = state.get("daily_feedback") or {}
    exercise       = str(state.get("exercise") or "운동")
    print(f"  ▶ exercise={exercise}")
    print(f"  ▶ daily_feedback keys: {list(daily_feedback.keys())}")
    ...
    score_direction = daily_feedback.get("score_direction", "stable")
    print(f"  ▶ set_summaries={len(set_summaries)}건 | avg_score={avg_score} | score_direction={score_direction}")

    fallback = {"summary": "", "main_issue": "", "coaching": "", "next_action": ""}

    if not set_summaries:
        print("  ✗ set_summaries가 비어 있음 → fallback 반환 (LLM 호출 안 함)")
        print("="*60)
    ...
    direction_label  = {"improving": "향상", "declining": "하락", "stable": "유지"}.get(score_direction, "유지")
    avg_score_str    = f"{avg_score}점" if avg_score is not None else "없음"

    system_prompt = (
        "당신은 운동 자세 분석 전문 코치입니다.\n"
        "아래에 오늘 수행한 세트들의 자세 분석 결과가 주어집니다.\n"
        "반드시 다음 4단계 사고 과정을 순서대로 거친 뒤, 최종 JSON만 출력하세요.\n"
        "단계별 추론 내용은 출력하지 않습니다.\n\n"
        "## Step 1 — 반복 패턴 파악\n"
        "- 각 세트 피드백에서 동일하거나 유사한 오류가 몇 번 언급되는지 세어라.\n"
        "- 세트가 진행될수록 특정 문제가 심화되는지, 개선되는지 확인하라.\n\n"
        "## Step 2 — 점수 추이 분석\n"
        f"- 점수 추이: {score_trend_str} (전반·후반 비교 방향: {direction_label})\n"
        "- 점수 변화가 피로 누적, 집중력 저하, 기술 습득 중 어디에 해당하는지 판단하라.\n\n"
        "## Step 3 — 근본 원인 특정\n"
        "- Step 1·2에서 도출한 내용을 종합해 핵심 문제 1가지를 명확히 하라.\n"
        "  예) '반복적 허리 굽힘 → 코어 지구력 부족', '후반 점수 하락 → 피로로 인한 자세 붕괴'\n\n"
        "## Step 4 — 교정 코칭 수립\n"
        "- Step 3의 근본 원인을 해결할 교정 포인트 1~2가지를 도출하라.\n"
        "- 다음 세트·운동에서 바로 적용 가능한 구체적 행동 큐(cue)로 제시하라.\n\n"
        "## 최종 출력 (JSON만)\n"
        "위 4단계 추론을 마친 뒤 아래 JSON 형식으로만 답하세요:\n"
        '{"summary": "오늘 운동 전체를 점수 추이·주요 문제를 포함해 1~2문장으로 평가", '
        '"main_issue": "가장 반복된 핵심 문제 1가지 (짧고 명확하게)", '
        '"coaching": "근본 원인 기반의 구체적 교정 코칭 — 평가와 코칭을 통합해 2~3문장", '
        '"next_action": "다음 운동에서 즉시 적용할 단 1가지 실천 항목"}'
    )
    user_prompt = (
        f"운동 종목: {exercise}\n"
        f"총 세트: {total_sets}세트 | 평균 점수: {avg_score_str} | 점수 추이: {score_trend_str}\n\n"
        f"세트별 피드백:\n{sets_lines}"
    )

    print(f"  ▶ LLM 호출 준비 완료 | user_prompt 일부:\n    {user_prompt[:200]}")
    print("  ▶ LLM 호출 중...")
    try:
        response = get_llm().invoke([
            SystemMessage(content=system_prompt),
        ...
        raw = response.content
        print(f"  ✓ LLM 응답 수신 (타입={type(raw).__name__}, 길이={len(str(raw))})")
        if isinstance(raw, list):
            raw = "\n".join(
                part["text"] for part in raw if isinstance(part, dict) and "text" in part
            )
        ...
        json_match = re.search(r"\{[\s\S]*\}", str(raw).strip())
        if not json_match:
            print("  ✗ JSON 파싱 실패 (정규식 매칭 없음) → fallback 반환")
            print("="*60)
            return {"feedback_text": fallback}
        parsed = json.loads(json_match.group())
        result = {
        ...
        print(f"  ✓ feedback_text 생성 완료:")
        for k, v in result["feedback_text"].items():
            print(f"    {k}: {v[:80]}")
        print("="*60)
        return result
    except Exception as e:
...
if __name__ == "__main__":
    ...
    # ── run 모드 입력 ─────────────────────────────────────────
    NORMALIZED_POSE_PATH = "test_json/normalized.json"
    EXERCISE = "플랭크"
    CAMERA_VIEW = "측면"
    RULE_CONFIG_PATH = "config/plank_rule_config_mediapipe.json"
    ANALYSIS_OUTPUT_PATH = "outputs/test_normalized_analysis.json"
    ...
    if MODE in ("run", "both"):
        ...
        score_summary = analysis_result.get("score_summary") or {}
        print(f"overall_accuracy_pct     : {score_summary.get('overall_accuracy_pct')}")
        print(f"average_window_score_pct : {score_summary.get('average_window_score_pct')}")
        print(f"time_weighted_score_pct  : {score_summary.get('time_weighted_score_pct')}")
